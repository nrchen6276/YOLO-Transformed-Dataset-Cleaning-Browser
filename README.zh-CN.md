...
本仓库严格按照内部程序版本顺序发版。当前公开 release 与内部构建 `V1.8.1_202606041443` 对齐。
...
## 当前版本：V1.8.1_202606041443

`V1.8.1_202606041443` 属于 Tkinter 图源组筛选、数据集初始化、安全事务和性能止血线，严格接在内部 `V1.8_202606040320` 之后。

V1.8.1 新增或改进：

- 新增快速复核索引（FastReviewIndex），用于当前 review 目录的内存索引。
- 新增 quick preview 加载，让首批组在完整索引完成前即可渲染。
- 点击准备保持在当前组路径上，避免反复扫描大目录。
- 新增 scan/index/prepare 等阶段的性能探针。
- 运行日志分流到 Production/Test/Debug 等目录。
- 新增 `IN_PROGRESS_SYNCED` 标签同步状态，用于表示进行中目录标签完整。
- 限制缩略图预加载数量，减少大目录下的磁盘抢占。
- 继承 V1.8 的事务日志、恢复快照、review lock、casefold 目录安全、ID 初始化、缓存状态事务、数字小键盘快捷键、后台移动队列、缩略图缓存、过程日志、失败回滚、撤销、动态 `.rf.` 分组和校核报告导出。
- 测试覆盖：Windows 下 `32 tests OK, skipped=1`；跳过项是平台限制下无法同时创建 `done`/`Done` 兄弟目录的大小写碰撞场景。
...
V1.8.1 支持 YOLO 风格数据集树下的标准或临时复核目录：
