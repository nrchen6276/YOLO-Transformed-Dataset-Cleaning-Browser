...
class RuntimeLogger:
    def __init__(self, script_version: str, script_timecode: str, run_mode: str | None = None) -> None:
        self.script_version = script_version
        self.script_timecode = script_timecode
        self.run_mode = run_mode or detect_run_mode()
        self.run_id = datetime.now().strftime("%Y%m%d%H%M%S%f")[:-3]
        self.started_perf = time.perf_counter()
        self.lock = threading.Lock()
        self.programme_dir = self.resolve_programme_dir()
        self.log_dir = self.programme_dir / "Runtime_Logs" / runtime_log_subdir(self.run_mode)
        self.recovery_dir = self.programme_dir / "Recovery_Reports" / runtime_log_subdir(self.run_mode)
...
RUNTIME_RUN_MODE: str | None = None
...
def detect_run_mode() -> str:
    ...
    if env_mode:
        return env_mode
    argv_text = " ".join(sys.argv).casefold()
    if "unittest" in argv_text or "pytest" in argv_text or Path(sys.argv[0]).name.startswith("test_"):
        return "test"
    if "--audit-only" in argv_text:
        return "audit_only"
    return "gui_production"


def runtime_log_subdir(run_mode: str) -> str:
    mapping = {
        "test": "Test",
        "audit_only": "Production",
...
def set_runtime_run_mode(run_mode: str) -> None:
    global RUNTIME_RUN_MODE
    RUNTIME_RUN_MODE = run_mode


def ensure_runtime_logger(run_mode: str | None = None) -> RuntimeLogger:
    global RUNTIME_LOGGER
    requested_mode = run_mode or RUNTIME_RUN_MODE
    if RUNTIME_LOGGER is None:
        with RUNTIME_LOGGER_LOCK:
            if RUNTIME_LOGGER is None:
                RUNTIME_LOGGER = RuntimeLogger(SCRIPT_VERSION, SCRIPT_TIMECODE, requested_mode)
...
@dataclass
class FastReviewIndex:
    image_review_dir: Path
    label_review_dir: Path
    group_size: int
    root_groups_by_prefix: dict[str, list[Path]] = field(default_factory=dict)
    label_by_stem: dict[str, Path] = field(default_factory=dict)
    duplicate_label_stems: dict[str, list[Path]] = field(default_factory=dict)
    done_target_stems: set[str] = field(default_factory=set)
    out_target_stems: set[str] = field(default_factory=set)
    completed_prefixes: set[str] = field(default_factory=set)
    root_prefix_queue: list[str] = field(default_factory=list)
    audit_counts: AuditSummary | None = None
    ...
    def selectable_groups(self, exclude_prefixes: set[str] | None = None) -> list[tuple[str, list[Path]]]:
        exclude_prefixes = exclude_prefixes or set()
        return [
            (prefix, self.root_groups_by_prefix[prefix])
            for prefix in self.root_prefix_queue
            if prefix in self.root_groups_by_prefix and prefix not in exclude_prefixes
        ]

    def apply_queued_transaction(self, transaction: MoveTransaction) -> None:
        self.root_groups_by_prefix.pop(transaction.prefix, None)
        self.root_prefix_queue = [prefix for prefix in self.root_prefix_queue if prefix != transaction.prefix]
        self.completed_prefixes.add(transaction.prefix)
        for op in transaction.operations:
            if op.kind != "image":
                continue
            if op.role == "done":
                self.done_target_stems.add(op.target.stem)
            elif op.role == "out":
                self.out_target_stems.add(op.target.stem)
        if self.audit_counts is not None and transaction.group_size > 1:
            n = transaction.group_size
            self.audit_counts.root_image_count = max(0, self.audit_counts.root_image_count - n)
            self.audit_counts.root_prefix_count = max(0, self.audit_counts.root_prefix_count - 1)
            self.audit_counts.done_image_count += 1
            self.audit_counts.done_prefix_count += 1
            self.audit_counts.out_image_count += n - 1
            self.audit_counts.out_prefix_count += 1
            self.audit_counts.selectable_group_count = max(0, self.audit_counts.selectable_group_count - 1)
...
class AuditRefreshResult:
    ...
    fast_index: FastReviewIndex | None = None
    error: str = ""
...
def compute_label_sync_status(summary: AuditSummary, has_duplicate_labels: bool = False) -> str:
    if summary.label_position_sync_pass:
        return "PASS"
    if (
        summary.all_image_stems_have_label_somewhere
        and summary.missing_label_stem_count == 0
        and summary.orphan_label_stem_count == 0
        and not has_duplicate_labels
        and not summary.blocking_errors
    ):
...
def export_yolo_audit_markdown(audit: YoloDatasetAudit, output_root: Path | None = None) -> Path:
    ...
    timecode = datetime.now().strftime("%Y%m%d%H%M%S%f")[:-3]
...
def build_review_fast_state(image_review_dir: Path) -> ReviewFastState:
    ...
    log_event("build_review_fast_state_start", review_dir=str(image_review_dir))
    ...
    log_perf(
        "build_review_fast_state_done",
        started,
        review_dir=str(image_review_dir),
        label_count=sum(len(paths) for paths in state.label_index.values()),
        done_target_count=len(state.image_done_names),
        out_target_count=len(state.image_out_names),
    )
    return state


def build_label_unique_index(label_review_dir: Path) -> tuple[dict[str, Path], dict[str, list[Path]]]:
    label_index = build_label_index(label_review_dir)
    ...
    for stem, paths in label_index.items():
        ...
        if len(existing_paths) == 1:
            unique[stem] = existing_paths[0]
        elif len(existing_paths) > 1:
            duplicates[stem] = existing_paths
    return unique, duplicates


def build_fast_index_summary(
    image_review_dir: Path,
    label_review_dir: Path,
    group_size: int,
    img_root: list[Path],
    ...
    image_stems = stem_set(img_root + img_done + img_out)
    label_stems = set(label_by_stem) | set(duplicate_label_stems)
    missing_labels = image_stems - label_stems
    orphan_labels = label_stems - image_stems
    expected_out = max(group_size - 1, 0)
    label_position_sync = (
        not img_root
        and not sorted_top_files(label_review_dir, {LABEL_EXT})
        and len(img_done) == len([path for path in label_by_stem.values() if path.parent.name.casefold() == "done"])
        and len(img_out) == len([path for path in label_by_stem.values() if path.parent.name.casefold() == "out"])
        and stem_set(img_done).issubset(label_stems)
        and stem_set(img_out).issubset(label_stems)
    )
    errors = [issue_to_text(issue) for issue in issues if issue.severity == "ERROR"]
    if missing_labels:
        errors.append(f"ERROR_MISSING_LABEL: missing unique labels: {len(missing_labels)}")
    if duplicate_label_stems:
        errors.append(f"ERROR_DUPLICATE_LABEL: duplicate label stems: {len(duplicate_label_stems)}")
    warnings: list[str] = []
    if orphan_labels:
        warnings.append(f"WARN_ORPHAN_LABEL: orphan labels not matched by image stems: {len(orphan_labels)}")
    summary = AuditSummary(
        ...
        image_rule_pass=not issues and (set(out_by) == set(done_by) if done_by or out_by else True),
        label_position_sync_pass=label_position_sync,
        all_image_stems_have_label_somewhere=image_stems.issubset(label_stems),
        ...
        selectable_group_count=0,
        can_export_report=True,
    )
    summary.label_sync_status = compute_label_sync_status(summary, bool(duplicate_label_stems))
    return summary


def build_fast_review_index(image_review_dir: Path, preview_limit: int | None = None) -> FastReviewIndex:
    overall_started = time.perf_counter()
    log_event("build_fast_review_index_start", review_dir=str(image_review_dir), preview_limit=preview_limit)
    group_size, _id_root, label_review_dir = parse_review_dir(image_review_dir)
    validate_child_dirs(image_review_dir, label_review_dir, create_missing=True)

    started = time.perf_counter()
    img_root = sorted_top_files(image_review_dir, IMAGE_EXTS)
    log_perf("scan_root_images_done", started, review_dir=str(image_review_dir), file_count=len(img_root))
    ...
    log_perf("scan_done_out_done", started, review_dir=str(image_review_dir), done_count=len(img_done), out_count=len(img_out))

    started = time.perf_counter()
    label_by_stem, duplicate_label_stems = build_label_unique_index(label_review_dir)
    log_perf(
        "scan_labels_done",
        started,
        review_dir=str(image_review_dir),
        label_dir=str(label_review_dir),
        unique_count=len(label_by_stem),
        duplicate_count=len(duplicate_label_stems),
    )
    ...
    for prefix, members in root_by.items():
        if prefix.startswith("__NO_RF__::"):
            severity = "WARN" if auto_group_size else "ERROR"
            code = "WARN_MISSING_RF_PREFIX" if auto_group_size else "ERROR_MISSING_RF_PREFIX"
            blocks_selection = not auto_group_size
            issues.append(AuditIssue(severity, code, prefix, [path.name for path in members], "Missing .rf. prefix.", "Rename or remove the affected image.", blocks_selection))
        elif group_size > 1 and len(members) != group_size:
            issues.append(AuditIssue("ERROR", "ERROR_ROOT_GROUP_SIZE_MISMATCH", prefix, [path.name for path in members], f"Root group has {len(members)} images, expected {group_size}.", "Repair the group before selection.", True))
    for prefix, members in done_by.items():
        if len(members) != 1:
            issues.append(AuditIssue("ERROR", "ERROR_DONE_DUPLICATE_PREFIX", prefix, [path.name for path in members], "Done prefix does not contain exactly one representative.", "Keep one done source image.", True))
    for prefix, members in out_by.items():
        if group_size > 1 and len(members) != expected_out:
            issues.append(AuditIssue("ERROR", "ERROR_OUT_GROUP_SIZE_MISMATCH", prefix, [path.name for path in members], f"Out prefix has {len(members)} images, expected {expected_out}.", "Repair out folder for this prefix.", True))
    for prefix in sorted(set(root_by) & (set(done_by) | set(out_by))):
        issues.append(AuditIssue("ERROR", "ERROR_PREFIX_SPLIT_ACROSS_ROOT_DONE_OUT", prefix, [], "Prefix exists in root and done/out.", "Resolve the split prefix before selection.", True))
    if set(done_by) != set(out_by):
        issues.append(AuditIssue("ERROR", "ERROR_DONE_OUT_PREFIX_SET_MISMATCH", "", [], "Done/out prefix sets are not closed.", "Repair done/out prefix mismatch.", True))
    for stem, paths in duplicate_label_stems.items():
        issues.append(AuditIssue("ERROR", "ERROR_DUPLICATE_LABEL", "", [str(path) for path in paths], f"Duplicate label stem: {stem}.", "Keep exactly one label for this stem.", True))

    completed_prefixes = set(done_by) | set(out_by)
    valid_prefixes: list[str] = []
    valid_root_groups: dict[str, list[Path]] = {}
    for prefix, members in root_by.items():
        if prefix.startswith("__NO_RF__::") or prefix in completed_prefixes:
            continue
        ...
        valid_root_groups[prefix] = sorted(members, key=lambda item: item.name.lower())
        if preview_limit is not None and len(valid_prefixes) >= preview_limit:
            break

    summary = build_fast_index_summary(
        image_review_dir,
        label_review_dir,
        group_size,
        img_root,
    ...
    blocking_issues = [issue for issue in issues if issue.blocks_selection]
    summary.selectable_group_count = len(valid_prefixes)
    summary.can_select = bool(valid_prefixes) and (group_size > 1 or auto_group_size) and not blocking_issues
    summary.info = [issue for issue in issues if not issue.blocks_selection]

    index = FastReviewIndex(
        image_review_dir=image_review_dir,
        label_review_dir=label_review_dir,
        group_size=group_size,
        root_groups_by_prefix=valid_root_groups,
        label_by_stem=label_by_stem,
        duplicate_label_stems=duplicate_label_stems,
        done_target_stems=stem_set(img_done) | {stem for stem, path in label_by_stem.items() if path.parent.resolve(strict=False) == lab_done_dir.resolve(strict=False)},
        out_target_stems=stem_set(img_out) | {stem for stem, path in label_by_stem.items() if path.parent.resolve(strict=False) == lab_out_dir.resolve(strict=False)},
        completed_prefixes=completed_prefixes,
        root_prefix_queue=valid_prefixes,
        audit_counts=summary,
        ready_for_preview=bool(valid_prefixes),
        ready_for_commit=bool(valid_prefixes) and not blocking_issues and preview_limit is None,
        blocking_errors=blocking_issues,
    )
    log_perf(
        "build_fast_review_index_done",
        overall_started,
        review_dir=str(image_review_dir),
        preview_limit=preview_limit,
        ready_for_preview=index.ready_for_preview,
        ready_for_commit=index.ready_for_commit,
        root_prefix_count=len(root_by),
        selectable_prefix_count=len(valid_prefixes),
        blocking_error_count=len(blocking_issues),
    )
    return index


def build_quick_preview_index(image_review_dir: Path, preview_limit: int = 20) -> FastReviewIndex:
    started = time.perf_counter()
    group_size, _id_root, label_review_dir = parse_review_dir(image_review_dir)
    if not image_review_dir.exists() or not image_review_dir.is_dir():
        raise ManualReviewError(f"Image review dir does not exist: {image_review_dir}")
    groups: dict[str, list[Path]] = defaultdict(list)
    prefix_order: list[str] = []
    complete_prefixes: list[str] = []
    complete_prefix_set: set[str] = set()
    scanned = 0
    auto_group_size = group_size == 0
    if group_size == 1:
        return FastReviewIndex(image_review_dir, label_review_dir, group_size, ready_for_preview=False, ready_for_commit=False)
    try:
        with os.scandir(image_review_dir) as entries:
            for entry in entries:
                if not entry.is_file():
                    continue
                name_path = Path(entry.name)
                if name_path.suffix.lower() not in IMAGE_EXTS:
                    continue
                scanned += 1
                item = image_review_dir / entry.name
                prefix, _mode = get_prefix(item)
                if prefix is None:
                    continue
                if prefix not in groups:
                    prefix_order.append(prefix)
                groups[prefix].append(item)
                if not auto_group_size and len(groups[prefix]) == group_size and prefix not in complete_prefix_set:
                    complete_prefix_set.add(prefix)
                    complete_prefixes.append(prefix)
                if len(complete_prefixes) >= preview_limit:
                    break
    except OSError as exc:
        raise ManualReviewError(f"Failed to scan image review dir for quick preview: {exc}") from exc
    if auto_group_size:
        valid_prefixes = [
            prefix
            for prefix in prefix_order
            if not prefix.startswith("__NO_RF__::") and len(groups[prefix]) > 1
        ][:preview_limit]
    else:
        valid_prefixes = complete_prefixes[:preview_limit]
    valid_groups = {prefix: sorted(groups[prefix], key=lambda path: path.name.lower()) for prefix in valid_prefixes}
    summary = AuditSummary(
        review_name=image_review_dir.name,
        group_size=group_size,
        image_base=str(image_review_dir),
        label_base=str(label_review_dir),
        status=STATUS,
        warnings=["FAST_PREVIEW_ONLY: complete audit index is still building in background."],
        root_image_count=scanned,
        root_prefix_count=len(valid_prefixes),
        selectable_group_count=len(valid_prefixes),
        can_select=False,
        can_export_report=True,
        label_sync_status="INDEX_BUILDING",
    )
    index = FastReviewIndex(
        image_review_dir=image_review_dir,
        label_review_dir=label_review_dir,
        group_size=group_size,
        root_groups_by_prefix=valid_groups,
        root_prefix_queue=valid_prefixes,
        audit_counts=summary,
        ready_for_preview=bool(valid_prefixes),
    ...
    log_perf(
        "quick_preview_index_done",
        started,
        review_dir=str(image_review_dir),
        scanned_image_count=scanned,
        preview_prefix_count=len(valid_prefixes),
        group_size=group_size,
    )
    return index


def prepare_transaction_from_fast_index(index: FastReviewIndex, prefix: str, selected_image: Path) -> MoveTransaction:
    started = time.perf_counter()
    if not index.ready_for_commit:
        raise ManualReviewError("FastReviewIndex is still building; preview is available but commit is not ready.")
    members = index.root_groups_by_prefix.get(prefix, [])
    if index.group_size > 1 and len(members) != index.group_size:
        raise ManualReviewError(
            f"ERROR_ROOT_GROUP_SIZE_MISMATCH: prefix {prefix} has {len(members)} root images, expected {index.group_size}."
        )
    selected_resolved = selected_image.resolve(strict=False)
    member_by_resolved = {path.resolve(strict=False): path for path in members}
    if selected_resolved not in member_by_resolved:
        raise ManualReviewError(f"Selected image is not part of current prefix: {selected_image}")

    img_done_dir = canonical_child_dir(index.image_review_dir, "done")
    img_out_dir = canonical_child_dir(index.image_review_dir, "out")
    lab_done_dir = canonical_child_dir(index.label_review_dir, "done")
    lab_out_dir = canonical_child_dir(index.label_review_dir, "out")
    operations: list[MoveOperation] = []

    lookup_started = time.perf_counter()
    labels: dict[str, Path] = {}
    for image in members:
        label = index.label_by_stem.get(image.stem)
        if label is None:
            raise ManualReviewError(f"ERROR_MISSING_LABEL: missing unique label for stem {image.stem}")
        labels[image.stem] = label
    log_perf("label_lookup_done", lookup_started, prefix=prefix, label_count=len(labels))

    conflict_started = time.perf_counter()
    for image in members:
        role = "done" if image.resolve(strict=False) == selected_resolved else "out"
        ...
        target_stems = index.done_target_stems if role == "done" else index.out_target_stems
        if target_image.stem in target_stems:
            raise ManualReviewError(f"ERROR_TARGET_EXISTS: target image stem already indexed: {target_image}")
        source_label = labels[image.stem]
        label_no_op = source_label.resolve(strict=False) == target_label.resolve(strict=False)
        if target_label.stem in target_stems and not label_no_op:
            raise ManualReviewError(f"ERROR_TARGET_EXISTS: target label stem already indexed: {target_label}")
        if not image.exists():
            raise ManualReviewError(f"ERROR_SOURCE_MISSING: source image missing: {image}")
        if not source_label.exists():
            raise ManualReviewError(f"ERROR_SOURCE_MISSING: source label missing: {source_label}")
    ...
    log_perf("target_conflict_check_done", conflict_started, prefix=prefix, operation_count=len(operations))

    transaction = MoveTransaction(
        review_name=index.image_review_dir.name,
        group_size=len(members),
        prefix=prefix,
        selected_stem=selected_image.stem,
        operations=operations,
    )
    log_perf("prepare_transaction_done", started, prefix=prefix, operation_count=len(operations), mode="fast_index")
    return transaction
...
def audit_review_dir(
    ...
    log_event("audit_review_dir_start", review_dir=str(image_review_dir))
    ...
    if invalid_root and auto_group_size:
        warnings.append(f"AUTO_GROUP_SIZE: root contains files without exact .rf. marker; excluded from selectable queue: {len(invalid_root)}")
    if invalid_root and not auto_group_size:
    ...
    summary = AuditSummary(
        ...
        can_select=bool(selectable_prefixes) and (group_size > 1 or auto_group_size) and not errors,
    )
    ...
    audit_issues = build_audit_issues(summary, rows, severity="ERROR")
    summary.blocking_errors = [issue for issue in audit_issues if issue.blocks_selection]
    summary.info = [issue for issue in audit_issues if not issue.blocks_selection]
    summary.can_select = bool(selectable_prefixes) and (group_size > 1 or auto_group_size) and not summary.blocking_errors
    summary.label_sync_status = compute_label_sync_status(summary, bool(duplicate_label_stems))
    log_perf(
        "audit_review_dir_done",
        audit_started,
        review_dir=str(image_review_dir),
        root_image_count=summary.root_image_count,
        done_image_count=summary.done_image_count,
        out_image_count=summary.out_image_count,
        label_recursive_total_count=summary.label_recursive_total_count,
        prefix_row_count=len(rows),
        blocking_error_count=len(summary.blocking_errors),
    )
    return summary, rows
    ...
    for row in rows:
        ...
        for bit in row.audit_status.split(";"):
            if bit.startswith("root_image_count_"):
                ...
                code = "WARN_MISSING_RF_PREFIX" if summary.group_size == 0 else "ERROR_MISSING_RF_PREFIX"
            else:
                code = "ERROR_AUDIT_RULE"
            issue_severity = "WARN" if summary.group_size == 0 and bit == "missing_rf_marker" else severity
            blocks_selection = not (summary.group_size == 0 and bit == "missing_rf_marker")
            issues.append(
                AuditIssue(
...
def selectable_groups(image_review_dir: Path) -> list[tuple[str, list[Path]]]:
    started = time.perf_counter()
    log_event("selectable_groups_start", review_dir=str(image_review_dir))
    group_size, _id_root, label_review_dir = parse_review_dir(image_review_dir)
    if group_size == 1:
        log_perf("selectable_groups_done", started, review_dir=str(image_review_dir), group_count=0)
        return []
    validate_child_dirs(image_review_dir, label_review_dir, create_missing=True)
    img_root = sorted_top_files(image_review_dir, IMAGE_EXTS)
    ...
    log_perf("selectable_groups_done", started, review_dir=str(image_review_dir), group_count=len(groups))
    return groups
...
def scan_unfinished_transactions(
    ...
    for path in sorted(log_dir.rglob(f"file_transactions_{SCRIPT_VERSION}_*.jsonl")):
        try:
...
class ManualReviewPickerApp:
    def __init__(self, id_root: Path | None = None, review_dir: Path | None = None) -> None:
        ...
        self.fast_index_by_review: dict[Path, FastReviewIndex] = {}
        self.current_fast_index: FastReviewIndex | None = None
        ...
        self.preload_group_count = 5
        self.max_preload_images = 20
    ...
    def prepare_fast_transaction(self, prefix: str, image_path: Path) -> MoveTransaction:
        ...
        index = self.fast_index_by_review.get(key)
        if index is None:
            raise ManualReviewError("FastReviewIndex is not ready; preview is available but commit is still disabled.")
        return prepare_transaction_from_fast_index(index, prefix, image_path)
    ...
    def apply_transaction_to_fast_state(self, transaction: MoveTransaction) -> None:
        ...
        if index is not None:
            index.apply_queued_transaction(transaction)
        if state is None:
            return
        for op in transaction.operations:
            if op.kind == "image":
    ...
    def run_audit_refresh_worker(self, review_dir: Path) -> None:
        index_result = AuditRefreshResult(review_dir=review_dir, stage="index_ready")
        try:
            fast_index = build_fast_review_index(review_dir, preview_limit=None)
            index_result.fast_index = fast_index
            index_result.summary = fast_index.audit_counts
            index_result.groups = fast_index.selectable_groups()
            self.audit_event_queue.put(index_result)
        except Exception as exc:
            index_result.error = str(exc)
            self.audit_event_queue.put(index_result)
            return

        audit_result = AuditRefreshResult(review_dir=review_dir, stage="audit_ready")
        try:
            summary, rows = audit_review_dir(review_dir, create_missing=True)
            audit_result.summary = summary
            audit_result.rows = rows
        except Exception as exc:
            audit_result.error = str(exc)
        self.audit_event_queue.put(audit_result)
    ...
    def poll_audit_events(self) -> None:
        ...
        while True:
            try:
                ...
                break
            if result.stage == "audit_ready" or result.error:
                self.audit_refresh_running = False
            ...
            if result.error:
                ...
                self.update_queue_status()
                continue
            if result.stage == "index_ready":
                if result.fast_index is not None:
                    self.fast_index_by_review[result.review_dir] = result.fast_index
                    self.current_fast_index = result.fast_index
                    self.current_groups = self.filter_pending_groups(result.fast_index.selectable_groups(), self.current_review_dir)
                    self.current_group_members_by_prefix = {prefix: members for prefix, members in self.current_groups}
                    self.selection_blocked = bool(result.fast_index.blocking_errors)
                    self.audit_dirty = True
                    if result.summary is not None:
                        self.write_audit(result.summary, [])
                    self.formula_var.set(
                        "INDEX READY: commits enabled."
                        if result.fast_index.ready_for_commit
                        else "INDEX BLOCKED: inspect Issue Table."
                    )
                    self.update_queue_status()
                    self.render_current_group()
                continue
            if result.summary is not None and result.summary.blocking_errors:
                self.selection_blocked = True
            elif self.current_fast_index is not None:
                self.current_groups = self.filter_pending_groups(self.current_fast_index.selectable_groups(), self.current_review_dir)
                self.selection_blocked = False
            self.current_group_members_by_prefix = {prefix: members for prefix, members in self.current_groups}
            self.audit_dirty = False
            if result.summary is not None:
    ...
    def refresh_current_review(self) -> None:
        ...
        try:
            ...
            quick_index = build_quick_preview_index(self.current_review_dir, preview_limit=20)
            key = self.review_key(self.current_review_dir)
            self.fast_index_by_review[key] = quick_index
            self.current_fast_index = quick_index
            self.current_groups = self.filter_pending_groups(quick_index.selectable_groups(), self.current_review_dir)
            self.selection_blocked = False
            self.current_group_members_by_prefix = {prefix: members for prefix, members in self.current_groups}
            self.audit_dirty = True
            if quick_index.audit_counts is not None:
                self.write_audit(quick_index.audit_counts, [])
                self.formula_var.set("FAST PREVIEW: full audit index is building; image comparison is available.")
            log_perf("open_review_quick_preview_done", open_started, review_dir=str(self.current_review_dir), preview_groups=len(self.current_groups))
            self.render_current_group()
            self.request_audit_refresh()
            return
        except Exception as exc:
            self.current_groups = []
            self.current_group_members_by_prefix = {}
    ...
    def write_audit(self, summary: AuditSummary, rows: list[PrefixAuditRow] | None = None) -> None:
        ...
        if hasattr(self, "audit_summary_var"):
            label_status = summary.label_sync_status
        ...
        lines = [
            ...
            f"Label Sync: {summary.label_sync_status}",
            ...
            *issue_detail_lines,
            "",
            "Raw JSON is available from exported Audit Reports / Developer Logs.",
        ]
    ...
    def schedule_preload(self) -> None:
        ...
        offsets: list[int] = []
        image_budget = 0
        for offset in range(1, self.preload_group_count + 1):
            ...
            if image_budget + len(members) > self.max_preload_images and offsets:
                break
            offsets.append(offset)
            image_budget += len(members)
        self.preload_offsets = offsets
    ...
    def choose_source(self, image_path: Path) -> None:
        ...
        try:
            ...
            if "FastReviewIndex" not in str(exc):
                self.refresh_current_review()
            return
...
def main(argv: list[str] | None = None) -> int:
    ...
    set_runtime_run_mode("audit_only" if args.audit_only else "gui_production")
