...
SCRIPT_PATH = (
    ...
    / "Dataset"
    / "Select_Programme"
    / "CIVL7009_source_group_picker_gui_V1.8.1_202606041443.py"
)

spec = importlib.util.spec_from_file_location("manualreview_gui_v181", SCRIPT_PATH)
manualreview_gui_v181 = importlib.util.module_from_spec(spec)
assert spec and spec.loader
sys.modules["manualreview_gui_v181"] = manualreview_gui_v181
spec.loader.exec_module(manualreview_gui_v181)
...
class ManualReviewSourcePickerTests(unittest.TestCase):
    def test_prefix_includes_rf_marker(self) -> None:
        prefix, mode = manualreview_gui_v181.get_prefix(
    ...
    def test_id_root_scans_only_manualreview_dirs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            dirs = manualreview_gui_v181.review_dirs_for_id_root(root)
    ...
    def test_group_size_1_missing_label_dir_is_readonly_stat_not_error(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            summary, _rows = manualreview_gui_v181.audit_review_dir(img)
            ...
            self.assertEqual(manualreview_gui_v181.selectable_groups(img), [])
    ...
    def test_illegal_image_child_dir_blocks_audit(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (img / "Done_auto").mkdir()
            with self.assertRaises(manualreview_gui_v181.ManualReviewError):
                manualreview_gui_v181.audit_review_dir(img)
    ...
    def test_casefold_done_collision_blocks_audit(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (img / "done").mkdir()
            (img / "Done").mkdir()
            with self.assertRaisesRegex(manualreview_gui_v181.ManualReviewError, "ERROR_CASE_COLLISION"):
                manualreview_gui_v181.audit_review_dir(img)
    ...
    def test_single_case_variant_done_folder_is_reused(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            summary, _rows = manualreview_gui_v181.audit_review_dir(img, create_missing=True)
            self.assertEqual(summary.root_prefix_count, 1)
    ...
    def test_non_manualreview_target_folder_is_not_id_scan_candidate(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for suffix in ["a", "b", "c", "d"]:
                ...
                write_image(path)
                write_label(lab / f"{path.stem}.txt")
            self.assertEqual(manualreview_gui_v181.review_dirs_for_id_root(root), [])
    ...
    def test_out_prefix_missing_fails_image_rule(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_image(done)
            write_label(lab / "done" / f"{done.stem}.txt")
            summary, rows = manualreview_gui_v181.audit_review_dir(img, create_missing=True)
    ...
    def test_gui_issue_details_include_prefix_and_filenames(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _summary, rows = manualreview_gui_v181.audit_review_dir(img, create_missing=True)
            details = "\n".join(manualreview_gui_v181.build_gui_issue_detail_lines(rows))
    ...
    def test_yolo_audit_images_labels_root_and_initialise(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            audit = manualreview_gui_v181.audit_yolo_dataset(root)
            ...
            result = manualreview_gui_v181.initialise_manualreview_from_yolo(audit)
    ...
    def test_click_transaction_moves_images_and_labels_from_auto_dirs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            transaction = manualreview_gui_v181.prepare_transaction(img, "case.rf.", b)
            manualreview_gui_v181.execute_transaction(transaction)
    ...
    def test_fast_transaction_uses_cached_members_and_label_index(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            label_index = manualreview_gui_v181.build_label_index(lab)
            transaction = manualreview_gui_v181.prepare_transaction_from_members(
                img,
                "case.rf.",
                members[1],
                members,
    ...
    def test_cached_state_transaction_uses_cached_targets_and_labels(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            state = manualreview_gui_v181.build_review_fast_state(img)
            transaction = manualreview_gui_v181.prepare_transaction_from_cached_state(
                img,
                "case.rf.",
                b,
                [a, b],
    ...
    def test_keypad_slots_follow_traditional_numpad_layout(self) -> None:
        slots2 = manualreview_gui_v181.keypad_slots_for_count(2)
        self.assertEqual([(slot.key, slot.row, slot.col) for slot in slots2], [("1", 0, 0), ("2", 0, 1)])

        slots4 = manualreview_gui_v181.keypad_slots_for_count(4)
        self.assertEqual(
            {slot.key: (slot.row, slot.col) for slot in slots4},
            {"1": (1, 0), "2": (1, 1), "3": (1, 2), "4": (0, 0)},
        )

        slots9 = manualreview_gui_v181.keypad_slots_for_count(9)
        self.assertEqual(
    ...
    def test_duplicate_label_blocks_transaction(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_label(lab / "case.rf.b.txt")
            with self.assertRaises(manualreview_gui_v181.ManualReviewError):
                manualreview_gui_v181.prepare_transaction(img, "case.rf.", img / "case.rf.a.jpg")
    ...
    def test_undo_transaction_restores_files(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_label(lab / "case.rf.a.txt")
            write_label(lab / "case.rf.b.txt")
            transaction = manualreview_gui_v181.execute_transaction(
                manualreview_gui_v181.prepare_transaction(img, "case.rf.", a)
            )
            manualreview_gui_v181.undo_transaction(transaction)
    ...
    def test_transaction_journal_records_planned_before_commit(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_label(lab / "case.rf.a.txt")
            write_label(lab / "case.rf.b.txt")
            transaction = manualreview_gui_v181.prepare_transaction(img, "case.rf.", a)
            manualreview_gui_v181.execute_transaction(transaction)
            logger = manualreview_gui_v181.ensure_runtime_logger()
            rows = [
                line
    ...
    def test_unfinished_transaction_scan_detects_planned_state(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_label(lab / "case.rf.b.txt")
            transaction = manualreview_gui_v181.prepare_transaction(img, "case.rf.", a)
            manualreview_gui_v181.write_transaction_journal(transaction, "PLANNED")
            unfinished = manualreview_gui_v181.scan_unfinished_transactions(review_dir=img)
    ...
    def test_review_lock_active_pid_blocks_second_lock(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = make_id_root(tmp)
            img, _lab = make_review(root, 2)
            first = manualreview_gui_v181.ReviewDirLock(img)
            first.acquire()
            try:
                second = manualreview_gui_v181.ReviewDirLock(img)
                with self.assertRaisesRegex(manualreview_gui_v181.ManualReviewError, "ERROR_ACTIVE_REVIEW_LOCK"):
                    second.acquire()
            finally:
                first.release()
    ...
    def test_stale_review_lock_can_be_cleared(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = make_id_root(tmp)
            img, _lab = make_review(root, 2)
            lock_path = img / manualreview_gui_v181.LOCK_FILENAME
            lock_path.write_text('{"pid": 0}', encoding="utf-8")
            lock = manualreview_gui_v181.ReviewDirLock(img)
            with self.assertRaisesRegex(manualreview_gui_v181.ManualReviewError, "ERROR_STALE_REVIEW_LOCK"):
    ...
    def test_label_sync_in_progress_synced_for_complete_root_groups(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = make_id_root(tmp)
            img, lab = make_review(root, 3)
            for suffix in ["a", "b", "c"]:
                ...
                write_label(lab / f"{path.stem}.txt")
            summary, _rows = manualreview_gui_v181.audit_review_dir(img, create_missing=True)
            self.assertEqual(summary.label_sync_status, "IN_PROGRESS_SYNCED")
            self.assertTrue(summary.can_select)

    def test_fast_review_index_prepare_and_incremental_counts(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = make_id_root(tmp)
            img, lab = make_review(root, 3)
            for group in range(2):
                for suffix in ["a", "b", "c"]:
                    path = img / f"case{group}.rf.{suffix}.jpg"
                    write_image(path)
                    write_label(lab / f"{path.stem}.txt")
            index = manualreview_gui_v181.build_fast_review_index(img)
            self.assertTrue(index.ready_for_commit)
            self.assertEqual(len(index.root_prefix_queue), 2)
            prefix = index.root_prefix_queue[0]
            selected = index.root_groups_by_prefix[prefix][0]
            transaction = manualreview_gui_v181.prepare_transaction_from_fast_index(index, prefix, selected)
            before_root = index.audit_counts.root_prefix_count
            index.apply_queued_transaction(transaction)
            self.assertNotIn(prefix, index.root_groups_by_prefix)
            self.assertEqual(index.audit_counts.root_prefix_count, before_root - 1)
            self.assertIn(selected.stem, index.done_target_stems)

    def test_large_dir_quick_preview_and_prepare_performance(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = make_id_root(tmp)
            img, lab = make_review(root, 3)
            for group in range(3000):
                for suffix in ["a", "b", "c"]:
                    path = img / f"g{group:04d}.rf.{suffix}.jpg"
                    write_image(path)
                    write_label(lab / f"{path.stem}.txt")
            started = time.perf_counter()
            preview = manualreview_gui_v181.build_quick_preview_index(img, preview_limit=20)
            quick_ms = (time.perf_counter() - started) * 1000.0
            self.assertLess(quick_ms, 3000.0)
            self.assertTrue(preview.ready_for_preview)
            index = manualreview_gui_v181.build_fast_review_index(img)
            durations = []
            for prefix in index.root_prefix_queue[:50]:
                selected = index.root_groups_by_prefix[prefix][0]
                started = time.perf_counter()
                manualreview_gui_v181.prepare_transaction_from_fast_index(index, prefix, selected)
                durations.append((time.perf_counter() - started) * 1000.0)
            p95 = statistics.quantiles(durations, n=20)[18]
            self.assertLess(p95, 150.0)

    def test_test_run_logs_are_split_to_test_subdir(self) -> None:
        logger = manualreview_gui_v181.ensure_runtime_logger()
        self.assertEqual(logger.run_mode, "test")
        self.assertIn("Runtime_Logs\\Test", str(logger.path))
    ...
    def test_background_runner_moves_transaction_without_blocking_caller(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            runner = manualreview_gui_v181.BackgroundMoveRunner(events)
            transaction = manualreview_gui_v181.prepare_transaction(img, "case.rf.", a)
            task = manualreview_gui_v181.QueuedMoveTask(1, img.resolve(), "case.rf.", a.stem, transaction)
            runner.enqueue(task)
    ...
    def test_export_audit_defaults_to_select_programme_reports(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for suffix in ["a", "b", "c"]:
                ...
                write_image(path)
                write_label(lab / f"{path.stem}.txt")
            run_dir = manualreview_gui_v181.export_audit_report(root)
            ...
            programme_dir = manualreview_gui_v181.RuntimeLogger.resolve_programme_dir()
            raw_exts = manualreview_gui_v181.IMAGE_EXTS
...
if __name__ == "__main__":
    try:
        ...
        manualreview_gui_v181.close_runtime_logger()
