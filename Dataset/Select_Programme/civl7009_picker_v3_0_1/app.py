...
class MainWindow(QMainWindow):
    ...
    def on_manual_index_done(self, generation: int, result: ManualObjectsIndexResult) -> None:
        ...
        if self.manual_row_summary_map:
            self.open_manual_group_from_summary(self.manual_row_summary_map[0])
    ...
    def save_manual_selection(self, review_status: str) -> None:
        ...
        try:
            ...
            if not self.stay_after_save_box.isChecked():
                self.open_next_unreviewed_manual_group(group.group_key)
        except Exception as exc:
            self.show_message(str(exc))
    ...
    def open_next_unreviewed_manual_group(self, current_group_key: str) -> None:
        if not self.manual_filtered_summaries:
            return
        start = 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    if "--help" in args:
        print("CIVL7009 Source Group Picker V3.0.1")
