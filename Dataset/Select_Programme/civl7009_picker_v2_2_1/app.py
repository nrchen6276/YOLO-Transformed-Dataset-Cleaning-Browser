...
class MainWindow(QMainWindow):
    ...
    def __init__(self, run_mode: str = "gui_production") -> None:
        ...
        self.logger = V221Logger(run_mode)
    ...
    def build_ui(self) -> None:
        ...
        self.stack.addWidget(self.simple_page("ID 初始化（Initialise）", "V2.2.1 保留初始化向导入口；写入动作仍须 dry-run 与 INIT 确认，原 YOLO 数据不移动、不删除。"))
    ...
    def apply_style(self) -> None:
        self.setStyleSheet(
            ...
            #statusChip[selected="true"] { background: rgba(239,64,34,0.14); color: #EF4022; }
            ...
            #imageCard[selected="true"] { border: 3px solid #EF4022; background: rgba(255,255,255,0.98); }
            #imagePreview { background: rgba(232,237,242,0.88); border: 2px solid transparent; border-radius: 10px; }
            #imagePreview[selected="true"] { border: 2px solid #EF4022; background: rgba(239,64,34,0.05); }
            #cardNumber { color: #EF4022; font-size: 24px; font-weight: 900; }
            #cardFilename { color: #17201C; font-weight: 700; }
            #cardStatus { color: #667085; }
            QPushButton { background: rgba(255,255,255,0.88); border: 1px solid rgba(9,68,56,0.25); border-radius: 8px; padding: 8px 12px; }
            ...
            #navButton { padding: 8px 14px; min-width: 72px; }
    ...
    def auto_enable_safe_gate_if_ready(self) -> None:
        ...
        if not self.auto_move_armed:
            self.progress_chip.setText("已就绪；请确认启动说明后启用自动移动")
            return
        ok, reason = self.can_enable_safe_gate()
        if ok:
            self.set_safe_gate_enabled("auto_ready")
            self.progress_chip.setText("自动移动已开启；单击/数字键直接提交")
            return
        self.safe_gate_enabled = False
        self.update_safe_gate()
        self.progress_chip.setText(f"只读预览：{reason}")
    ...
    def update_safe_gate(self) -> None:
        if self.safe_gate_enabled:
            ...
            self.status_badge.setText("自动移动\n准备中" if self.auto_move_armed else "READ-ONLY\nPREVIEW")
...
def create_app(argv: list[str] | None = None) -> QApplication:
    ...
    app.setApplicationName("CIVL7009 Source Group Picker V2.2.1")
    return app


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="CIVL7009 图源筛选器 V2.2.1")
