...
class MainWindow(QMainWindow):
    ...
    def build_ui(self) -> None:
        ...
        self.stack.addWidget(self.simple_page("ID 初始化（Initialise）", "V2.2.3 保留初始化向导入口；写入动作仍须 dry-run 与 INIT 确认，原 YOLO 数据不移动、不删除。"))
...
def create_app(argv: list[str] | None = None) -> QApplication:
    ...
    app.setApplicationName("CIVL7009 Source Group Picker V2.2.3")
    return app


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="CIVL7009 图源筛选器 V2.2.3")
