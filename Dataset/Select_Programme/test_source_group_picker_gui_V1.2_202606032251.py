...
SCRIPT_PATH = PACKAGE_ROOT / "CIVL7009_source_group_picker_gui_V1.2_202606032251.py"

spec = importlib.util.spec_from_file_location("manualreview_gui_v12", SCRIPT_PATH)
manualreview_gui_v12 = importlib.util.module_from_spec(spec)
assert spec and spec.loader
sys.modules["manualreview_gui_v12"] = manualreview_gui_v12
spec.loader.exec_module(manualreview_gui_v12)
...
class ManualReviewSourcePickerTests(unittest.TestCase):
    def test_prefix_includes_rf_marker(self) -> None:
        prefix, mode = manualreview_gui_v12.get_prefix(
    ...
    def test_id_root_scans_only_manualreview_dirs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            dirs = manualreview_gui_v12.review_dirs_for_id_root(root)
    ...
    def test_group_size_1_missing_label_dir_is_readonly_stat_not_error(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            summary, _rows = manualreview_gui_v12.audit_review_dir(img)
            ...
            self.assertEqual(manualreview_gui_v12.selectable_groups(img), [])
    ...
    def test_illegal_image_child_dir_blocks_audit(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (img / "Done_auto").mkdir()
            with self.assertRaises(manualreview_gui_v12.ManualReviewError):
                manualreview_gui_v12.audit_review_dir(img)
    ...
    def test_root_group_size_mismatch_is_warning_not_hard_error(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            summary, _rows = manualreview_gui_v12.audit_review_dir(img, create_missing=True)
    ...
    def test_arbitrary_target_folder_uses_dynamic_group_sizes(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            groups = dict(manualreview_gui_v12.selectable_groups(img))
    ...
    def test_out_prefix_missing_fails_image_rule(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_image(done)
            write_label(lab / "done" / f"{done.stem}.txt")
            summary, rows = manualreview_gui_v12.audit_review_dir(img, create_missing=True)
    ...
    def test_click_transaction_moves_images_and_labels_from_auto_dirs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            transaction = manualreview_gui_v12.prepare_transaction(img, "case.rf.", b)
            manualreview_gui_v12.execute_transaction(transaction)
    ...
    def test_duplicate_label_blocks_transaction(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_label(lab / "case.rf.b.txt")
            with self.assertRaises(manualreview_gui_v12.ManualReviewError):
                manualreview_gui_v12.prepare_transaction(img, "case.rf.", img / "case.rf.a.jpg")
    ...
    def test_undo_transaction_restores_files(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_label(lab / "case.rf.a.txt")
            write_label(lab / "case.rf.b.txt")
            transaction = manualreview_gui_v12.execute_transaction(
                manualreview_gui_v12.prepare_transaction(img, "case.rf.", a)
            )
            manualreview_gui_v12.undo_transaction(transaction)
    ...
    def test_background_runner_moves_transaction_without_blocking_caller(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            runner = manualreview_gui_v12.BackgroundMoveRunner(events)
            transaction = manualreview_gui_v12.prepare_transaction(img, "case.rf.", a)
            task = manualreview_gui_v12.QueuedMoveTask(1, img.resolve(), "case.rf.", a.stem, transaction)
            runner.enqueue(task)
    ...
    def test_export_audit_defaults_to_select_programme_reports(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for suffix in ["a", "b", "c"]:
                ...
                write_image(path)
                write_label(lab / f"{path.stem}.txt")
            run_dir = manualreview_gui_v12.export_audit_report(root)
