...
class PickerCoreFacade:
    ...
    def __init__(self, core_path: Path | None = None) -> None:
        self.candidate_paths = [Path(core_path).resolve()] if core_path is not None else [path.resolve() for path in CORE_CANDIDATES]
    ...
    def load(self) -> CoreLoadReport:
        ...
        try:
            ...
            self.core = module
            # V1.8.x is a monolithic Tkinter/core file and imports tkinter at
            # module import time. V3.0 records this as an audit note but keeps
            # the facade usable because the import does not start a Tk root,
            # mainloop, or mutate dataset state.
            ...
            return CoreLoadReport(
                ...
                error="" if ok else "core 符号、签名或 import side effect 检查失败",
                candidate_paths=candidate_text,
            )
