...
REVIEW_STATUSES = {"APPROVED", "SKIP", "AMBIGUOUS", "NEEDS_AGENT_CHECK", "ALL_OUT", "ALL_DONE"}
...
class ManualObjectsService:
    ...
    def save_selection(
        ...
        if review_status == "ALL_OUT":
            keep_set = set()
            remove_set = set(known_ids)
            if not notes:
                notes = "All candidate items in this group were marked as removal candidates by the reviewer."
        if review_status == "ALL_DONE":
            keep_set = set(known_ids)
            remove_set = set()
            if not notes:
                notes = "All candidate items in this group were marked as keep candidates by the reviewer."
        ...
        try:
            ...
            if tmp_path.exists():
                tmp_path.unlink()
        self.verify_saved_selection(target, group.group_key, review_status, sorted(keep_set), sorted(remove_set))
        group.selection = payload
        self.apply_selection_to_items(group)
        return target

    @staticmethod
    def verify_saved_selection(path: Path, group_key: str, review_status: str, keep_item_ids: list[str], remove_item_ids: list[str]) -> None:
        if not path.exists():
            raise RuntimeError(f"manual_selection.json was not created: {path}")
        try:
            saved = json.loads(path.read_text(encoding="utf-8"))
        except Exception as exc:
            raise RuntimeError(f"manual_selection.json could not be read back after save: {exc}") from exc
        expected_keep = sorted(str(item) for item in keep_item_ids)
        expected_remove = sorted(str(item) for item in remove_item_ids)
        ...
        problems: list[str] = []
        if str(saved.get("group_key", "")) != str(group_key):
            problems.append(f"group_key={saved.get('group_key')!r} != {group_key!r}")
        ...
        if problems:
            raise RuntimeError("manual_selection.json read-back verification failed: " + "; ".join(problems))
