...
SCRIPT_PATH = (
    ...
    / "Dataset"
    / "Select_Programme"
    / "CIVL7009_source_group_picker_qt_V1.9.1_202606041705.py"
)

spec = importlib.util.spec_from_file_location("source_group_picker_qt_v191", SCRIPT_PATH)
source_group_picker_qt_v191 = importlib.util.module_from_spec(spec)
assert spec and spec.loader
sys.modules["source_group_picker_qt_v191"] = source_group_picker_qt_v191
spec.loader.exec_module(source_group_picker_qt_v191)
...
class SourceGroupPickerQtV19Tests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.app = source_group_picker_qt_v191.create_app([])

    def make_facade(self) -> tuple[object, object]:
        logger = source_group_picker_qt_v191.V19Logger("test")
        facade = source_group_picker_qt_v191.PickerCoreFacade(logger, developer_mode=True)
        report = facade.load()
    ...
    def test_asset_manifest_declares_no_dataset_images(self) -> None:
        ...
        for asset in data["assets"]:
            ...
            self.assertEqual(source_group_picker_qt_v191.sha256_file(path), asset["sha256"])
    ...
    def test_core_facade_fails_closed_for_missing_symbols(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            logger = source_group_picker_qt_v191.V19Logger("test")
            facade = source_group_picker_qt_v191.PickerCoreFacade(logger, explicit_core_path=bad_core)
            try:
                report = facade.load()
    ...
    def test_safe_gate_off_preview_does_not_move_or_create_transaction_journal(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            before = source_group_picker_qt_v191.file_tree_fingerprint(Path(tmp))
            logger, facade = self.make_facade()
            window = source_group_picker_qt_v191.MainWindow(facade, logger)
            window.show()
            try:
                ...
                after = source_group_picker_qt_v191.file_tree_fingerprint(Path(tmp))
    ...
    def test_open_review_progress_and_ready_state(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertTrue(window.safe_gate_enabled)
                self.assertIn("文件移动已启用", window.banner.text())
                self.assertIn("已就绪", window.progress_chip.text())
            finally:
                window.close()
                process_events(self.app, 120)

    def test_ui_defaults_are_chinese_and_click_commit_mode(self) -> None:
        logger, facade = self.make_facade()
        window = source_group_picker_qt_v191.MainWindow(facade, logger)
        window.show()
        try:
            self.assertTrue(window.click_mode_commit)
            self.assertIn("图源人工筛选器", window.windowTitle())
            self.assertIn("选择 ID 根目录", window.open_id_btn.text())
            self.assertIn("选择筛选目录", window.open_review_btn.text())
            self.assertIn("单击图片即提交", window.mode_box.itemText(0))
            self.assertIn("只读预览", window.banner.text())
            self.assertEqual(window.kpi_title("Root remaining"), "剩余组")
        finally:
            window.close()
            logger.close()
            process_events(self.app, 80)
