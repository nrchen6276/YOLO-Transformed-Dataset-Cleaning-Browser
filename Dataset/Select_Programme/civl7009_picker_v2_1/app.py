...
class MainWindow(QMainWindow):
    ...
    def __init__(self, run_mode: str = "gui_production") -> None:
        ...
        self.logger = V21Logger(run_mode)
    ...
    def build_ui(self) -> None:
        ...
        self.stack.addWidget(self.simple_page("ID 初始化（Initialise）", "V2.1 保留初始化向导入口；写入动作仍须 dry-run 与 INIT 确认，原 YOLO 数据不移动、不删除。"))
    ...
    def build_sidebar(self) -> QWidget:
        ...
        layout.addStretch(1)
        self.status_badge = QLabel("READ-ONLY\nPREVIEW")
    ...
    def apply_style(self) -> None:
        self.setStyleSheet(
            """
            QMainWindow, QWidget { background: #EEF2F6; color: #17201C; font-size: 13px; font-family: "Microsoft YaHei UI", "Segoe UI", sans-serif; }
            #commandBar, #sidebar, #glassPanel, #stagePanel { background: rgba(255,255,255,0.76); border: 1px solid rgba(255,255,255,0.92); border-radius: 22px; }
            #sidebar { background: rgba(9,68,56,0.10); }
            #appTitle { font-size: 20px; font-weight: 800; color: #094438; }
            #sectionTitle { font-size: 22px; font-weight: 800; color: #094438; }
            #progressChip, #safeBadge, #statusChip { padding: 8px 12px; border-radius: 12px; background: rgba(209,193,141,0.24); color: #094438; font-weight: 700; }
            #safeBadge[enabledMoves="true"] { background: rgba(239,64,34,0.18); color: #EF4022; }
            #context { color: #667085; }
            #kpiCard { background: rgba(255,255,255,0.82); border: 1px solid rgba(209,193,141,0.55); border-radius: 16px; }
            #kpiValue { font-size: 22px; font-weight: 800; color: #094438; }
            #overlay { background: rgba(253,253,253,0.88); border: 1px solid rgba(255,255,255,0.95); border-radius: 18px; }
            #imageCard { background: rgba(255,255,255,0.82); border: 2px solid rgba(209,193,141,0.42); border-radius: 18px; }
            #imageCard[selected="true"] { border: 3px solid #009CD5; background: rgba(255,255,255,0.98); }
            #imagePreview { background: rgba(232,237,242,0.88); border-radius: 14px; }
            #cardNumber { color: #EF4022; font-size: 24px; font-weight: 900; }
            #cardFilename { color: #17201C; font-weight: 700; }
            #cardStatus { color: #667085; }
            QPushButton { background: rgba(255,255,255,0.88); border: 1px solid rgba(9,68,56,0.25); border-radius: 12px; padding: 8px 12px; }
            ...
            #navButton { text-align: left; padding: 10px; }
            #navButton:checked { background: rgba(9,68,56,0.16); color: #094438; font-weight: 800; }
            QTableView, QTextEdit { background: rgba(255,255,255,0.70); border: 1px solid rgba(9,68,56,0.10); border-radius: 12px; }
            """
        )
    ...
    def on_quick_ready(self, generation: int, index: Any) -> None:
        ...
        self.overlay.hide()
        self.progress_chip.setText("已可预览；完整索引完成前禁止提交")
        self.update_kpis()
    ...
    def on_index_ready(self, generation: int, lock: Any, index: Any, summary: Any, rows: Any, recovery: list[Any]) -> None:
        ...
        self.update_formula()
        self.render_current_group()
        self.progress_chip.setText("已就绪；READ-ONLY PREVIEW，开启 Safe Gate 后可提交")
    ...
    def set_progress(self, phase: str, detail: str) -> None:
        self.overlay.show()
        self.overlay_text.setText(f"{phase}\n{detail}")
        self.progress_chip.setText(phase)
    ...
    def toggle_safe_gate(self) -> None:
        ...
        if not ok:
            ...
            return
        text, accepted = QInputDialog.getText(self, "启用文件移动", f"当前筛选目录：\n{self.current_review_dir}\n\n输入 MOVE 后启用真实文件移动：")
        if accepted and text.strip() == "MOVE":
            self.set_safe_gate_enabled("manual")
        else:
            self.add_recent("已取消启用文件移动。")

    def enable_safe_gate_for_tests(self, confirmation: str = "MOVE") -> bool:
        ok, reason = self.can_enable_safe_gate()
        if ok and confirmation.strip() == "MOVE":
    ...
    def update_safe_gate(self) -> None:
        if self.safe_gate_enabled:
            self.status_badge.setText("FILE MOVES\nENABLED")
            self.status_badge.setProperty("enabledMoves", True)
            self.safe_gate_btn.setText("关闭文件移动")
        else:
            self.status_badge.setText("READ-ONLY\nPREVIEW")
    ...
    def undo_last_transaction(self) -> None:
        ...
        try:
            ...
            self.add_recent("上一组已撤销；请重新打开当前筛选目录以刷新索引。")
            ...
            self.update_safe_gate()
            self.show_empty_state()
            self.progress_chip.setText("撤销完成；请重新打开目录刷新")
        except Exception as exc:
    ...
    def update_formula(self) -> None:
        ...
        if s is None:
            ...
            return
        n = getattr(s, "group_size", getattr(s, "group_size_n", 0)) or getattr(s, "expected_out_per_done_group", 0) + 1
    ...
    def update_context(self) -> None:
        self.context_label.setText(
            f"ID: {self.id_root or '未选择'} | 筛选目录: {self.current_review_dir or '未选择'}"
        )
...
def create_app(argv: list[str] | None = None) -> QApplication:
    ...
    app.setApplicationName("CIVL7009 Source Group Picker V2.1")
    return app


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="CIVL7009 图源筛选器 V2.1")
