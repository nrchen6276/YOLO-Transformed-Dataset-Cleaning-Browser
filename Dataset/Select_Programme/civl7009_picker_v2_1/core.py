...
class PickerCoreFacade:
    ...
    def __init__(self, core_path: Path | None = None) -> None:
        self.core_path = (core_path or CORE_PATH).resolve()
        self.core: Any | None = None
        self.report = self.load()

    def load(self) -> CoreLoadReport:
        if not self.core_path.exists():
            return CoreLoadReport(ok=False, path=self.core_path, error=f"core 文件不存在: {self.core_path}")
        ...
        try:
            ...
            if not before_tk and after_tk:
                side_effect_note.append("core import loaded tkinter modules")
            return CoreLoadReport(
                ok=not missing and not signature_errors,
                path=self.core_path,
                core_sha256=sha256_file(self.core_path),
                missing_symbols=missing,
                signature_errors=signature_errors + side_effect_note,
                error="" if not missing and not signature_errors else "core 符号或签名检查失败",
            )
        except Exception as exc:
            return CoreLoadReport(ok=False, path=self.core_path, error=str(exc))
