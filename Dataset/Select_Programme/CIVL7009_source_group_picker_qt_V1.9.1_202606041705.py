...
class OpenReviewWorker(QRunnable):
    ...
    def run(self) -> None:
        ...
        lock = None
        try:
            self.signals.progress.emit(self.generation, "正在获取筛选目录锁", "检查是否已有程序正在处理该目录")
            ...
            self.signals.progress.emit(self.generation, "正在检查恢复状态", "检查是否存在未完成事务")
            recovery = core.scan_unfinished_transactions(review_dir=self.review_dir)
            if self.cancelled:
                lock.release()
                return

            self.signals.progress.emit(self.generation, "正在准备快速预览", "扫描第一批完整图片组")
            quick = core.build_quick_preview_index(self.review_dir, preview_limit=20)
            self.signals.quick_ready.emit(self.generation, quick)
            self.signals.progress.emit(self.generation, "首组图片已就绪", "可以预览；完整索引完成后自动启用移动")
            if self.cancelled:
                lock.release()
                return

            self.signals.progress.emit(self.generation, "正在构建快速索引", "建立可提交的 FastReviewIndex")
            index = core.build_fast_review_index(self.review_dir)
            if self.cancelled:
                lock.release()
                return

            self.signals.progress.emit(self.generation, "正在运行完整校核", "校核 root/done/out 与标签同步")
            summary, rows = core.audit_review_dir(self.review_dir, create_missing=False)
            self.signals.progress.emit(self.generation, "可以开始筛选", "校核完成；文件移动将自动启用")
...
class ThumbnailWorker(QRunnable):
    ...
    def run(self) -> None:
        if Image is None:
            self.signals.thumbnail_ready.emit(self.generation, str(self.image_path), "", b"", 0, 0, "Pillow 不可用")
            return
        try:
...
class MainWindow(QMainWindow):
    def __init__(self, facade: PickerCoreFacade, logger: V19Logger) -> None:
        ...
        self.auto_move_notice_shown = False
        self.notice_boxes: list[QMessageBox] = []
        ...
        self.issue_model = TableModel(["等级", "前缀", "代码", "说明", "建议处理"])
        self.queue_model = TableModel(["任务", "前缀", "选中图", "状态", "错误"])
        self.review_model = TableModel(["筛选目录", "根部", "完成", "异常", "标签同步"])
        self.cards: list[ImageCard] = []
        self.thumbnail_cache: dict[str, QImage] = {}
        self.open_worker: OpenReviewWorker | None = None

        self.setWindowTitle(f"CIVL7009 图源人工筛选器 {UI_VERSION}")
    ...
    def build_ui(self) -> None:
        ...
        self.banner = QLabel("只读预览")
        self.banner.setObjectName("safeBanner")
        main.addWidget(self.banner)

        toolbar = QHBoxLayout()
        self.open_id_btn = QPushButton("选择 ID 根目录")
        self.open_review_btn = QPushButton("选择筛选目录")
        self.safe_gate_btn = QPushButton("暂停文件移动")
        self.safe_gate_btn.setObjectName("dangerButton")
        self.export_btn = QPushButton("导出校核")
        self.undo_btn = QPushButton("撤销上一组")
        self.undo_btn.setEnabled(False)
        self.mode_box = QComboBox()
        self.mode_box.addItems(["单击图片即提交（推荐）", "单击仅预览，按 Enter 提交"])
    ...
    def kpi_title(self, key: str) -> str:
        titles = {
            "Root remaining": "剩余组",
            "Current group": "当前组",
            "Done groups": "图源组",
    ...
    def build_manual_tab(self) -> QWidget:
        ...
        sidebar_layout.addWidget(QLabel("目录大盘"))
    ...
    def build_initialisation_tab(self) -> QWidget:
        ...
        msg = QLabel(
            "本版本只完整移植人工筛选页。ID 初始化写入能力仍由 V1.8.1 fallback 负责；"
            "V1.9.1 不在这里执行 YOLO 初始化复制或移动。"
        )
        ...
        fallback.setText(
            "回退入口边界：\n"
            "- 本 V1.9.1 页面只读展示，不执行初始化写入。\n"
            "- 不在此处复制、移动或改写 YOLO 数据集文件。\n"
            f"- V1.8.1 回退入口源文件：{PROGRAMME_DIR / CORE_FILENAME}\n"
            "- 若后续启动回退入口，必须先显示 id_root、程序路径与写入风险确认。"
        )
    ...
    def show_core_status(self) -> None:
        ...
        if report.ok:
            status = f"后端核心：{self.logger.core_meta.get('core_version')} {self.logger.core_meta.get('core_timecode')}"
            if report.developer_override:
                status += "\n开发模式核心覆盖"
            status += f"\nSHA256: {report.core_sha256[:16]}..."
        else:
            status = f"后端核心加载失败\n{report.error}\n缺失符号：{', '.join(report.missing_symbols)}"
    ...
    def choose_id_root(self) -> None:
        root = QFileDialog.getExistingDirectory(self, "选择 Source_Archive/<ID> 根目录")
        ...
        try:
            ...
            self.add_recent(f"已读取 ID 根目录：{root}")
        except Exception as exc:
            self.add_issue("错误", "", "ID_ROOT_FAILED", str(exc), "检查 images/labels 目录结构。")

    def choose_review_dir(self) -> None:
        path = QFileDialog.getExistingDirectory(self, "选择 images/ManualReview_GroupSize_N 目录")
        if path:
    ...
    def open_review_dir(self, review_dir: Path) -> None:
        ...
        self.cards = []
        self.optimistic_transaction_ids.clear()
        self.auto_move_notice_shown = False
    ...
    def on_quick_ready(self, generation: int, index: Any) -> None:
        ...
        self.overlay.hide()
        self.progress_chip.setText("已可预览；完整索引完成前暂不提交")
        self.update_kpis()
        self.add_recent("快速预览已就绪")
    ...
    def on_index_ready(self, generation: int, lock: Any, index: Any, summary: Any, rows: Any, recovery: list[Any]) -> None:
        ...
        self.render_current_group()
        self.auto_enable_moves_if_ready()
        if self.safe_gate_enabled:
            self.progress_chip.setText("已就绪；文件移动已自动启用")
        else:
            self.progress_chip.setText("已就绪；仍为只读预览")
        self.add_recent("FastReviewIndex 与完整校核已完成")

    @Slot(int, str, str)
    def on_worker_error(self, generation: int, code: str, message: str) -> None:
        if generation != self.generation:
            return
        self.add_issue("错误", "", code, message, "处理该问题后再继续。")
    ...
    def on_thumbnail_ready(self, generation: int, path_text: str, key: str, payload: bytes, width: int, height: int, error: str) -> None:
        if generation != self.generation:
            return
        if error:
            self.add_issue("警告", "", "THUMBNAIL_FAILED", f"{path_text}: {error}", "需要细看时打开 100% 原图查看器。")
            return
    ...
    def select_card(self, index: int) -> None:
        ...
        self.add_recent(f"已选中预览 [{index + 1}] {self.cards[index].image_path.name}")
    ...
    def show_disabled_reason(self, reason: str | None = None) -> None:
        reason = reason or self.disabled_reason() or "当前操作不可用"
        QMessageBox.information(self, "操作暂不可用", reason)
        self.add_recent(f"操作被阻止：{reason}")
    ...
    def toggle_safe_gate(self) -> None:
        if self.safe_gate_enabled:
            ...
            self.add_recent("文件移动已暂停；已进入后台队列的任务会继续完成。")
            self.update_safe_gate_banner()
            self.update_commit_enabled()
            return
        ...
        box.setWindowTitle("启用文件移动")
        box.setText(
            f"即将允许真实文件移动。\n\n当前筛选目录：\n{self.current_review_dir}\n\n"
            "启用后，单击图片会把选中图移入 done，同组其他图移入 out；后台队列会继续执行已入队任务。"
        )
        enable_button = box.addButton("启用文件移动", QMessageBox.AcceptRole)
        cancel_button = box.addButton("取消", QMessageBox.RejectRole)
        box.setDefaultButton(enable_button)
        box.exec()
        if box.clickedButton() != enable_button:
            self.add_recent("已取消启用文件移动。")
            return
        self.enable_safe_gate_for_tests("MOVE", source="manual")

    def enable_safe_gate_for_tests(self, confirmation: str, source: str = "test") -> bool:
        ok, reason = self.can_enable_safe_gate()
        if not ok or confirmation.strip() != "MOVE":
            self.logger.event("safe_gate_enable_blocked", reason=reason or "MOVE 确认缺失")
            return False
        self.set_safe_gate_enabled(source=source)
        return True

    def set_safe_gate_enabled(self, source: str) -> None:
        self.safe_gate_enabled = True
        self.logger.event("safe_gate_enabled", review_dir=str(self.current_review_dir), source=source)
        self.add_recent("文件移动已启用：单击图片会提交当前组。")
        self.update_safe_gate_banner()
        self.update_commit_enabled()

    def auto_enable_moves_if_ready(self) -> None:
        ok, reason = self.can_enable_safe_gate()
        if not ok:
            self.logger.event("safe_gate_auto_enable_blocked", reason=reason)
            self.add_recent(f"文件移动暂未自动启用：{reason}")
            self.update_safe_gate_banner()
            self.update_commit_enabled()
            return
        self.set_safe_gate_enabled(source="auto_ready")
        if os.environ.get("CIVL7009_PICKER_TEST_MODE"):
            return
        if self.auto_move_notice_shown:
            return
        self.auto_move_notice_shown = True
        box = QMessageBox(self)
        box.setWindowTitle("文件移动已自动启用")
        box.setText(
            "索引、目录锁、恢复扫描和完整校核均已通过。\n\n"
            "当前进入文件移动模式：单击图片会提交当前组，前台会立即切到下一组，后台继续移动文件。"
        )
        box.setStandardButtons(QMessageBox.Ok)
        box.setModal(False)
        self.notice_boxes.append(box)

        def _cleanup() -> None:
            if box in self.notice_boxes:
                self.notice_boxes.remove(box)
            box.deleteLater()
    ...
    def update_safe_gate_banner(self) -> None:
        if self.safe_gate_enabled:
            self.banner.setText("文件移动已启用")
            self.banner.setProperty("enabledMoves", True)
            self.safe_gate_btn.setText("暂停文件移动")
        else:
            override = "  |  开发模式核心覆盖" if self.facade.report.developer_override else ""
            self.banner.setText(f"只读预览{override}")
            self.banner.setProperty("enabledMoves", False)
            self.safe_gate_btn.setText("启用文件移动")
    ...
    def dry_run_transaction(self) -> tuple[Any | None, str]:
        ...
        summary = [
            "dry-run 事务摘要：",
            f"前缀：{prefix}",
            f"选中图片/标签 -> done：{len(done_items)} 项操作",
            f"同组变换图片/标签 -> out：{len(out_items)} 项操作",
            "目标冲突检查：通过",
        ]
        return transaction, "\n".join(summary)
    ...
    def commit_selected(self) -> None:
        ...
        if transaction is None:
            self.add_issue("错误", "", "DRY_RUN_FAILED", summary, "修复该组问题后再移动文件。")
        ...
        if self.fast_index is not None:
            try:
                ...
                self.update_formula()
                self.logger.perf(
                    "optimistic_next_group_done",
                    started,
                    prefix=transaction.prefix,
                    remaining_groups=len(self.current_groups),
                )
            except Exception as exc:
                self.add_issue("错误", transaction.prefix, "OPTIMISTIC_UPDATE_FAILED", str(exc), "等待后台队列完成后重新打开目录。")
                self.logger.event("optimistic_next_group_failed", prefix=transaction.prefix, error=str(exc))
        self.update_commit_enabled()
        self.logger.perf("commit_selected_enqueued", started, prefix=transaction.prefix, selected_stem=transaction.selected_stem)
    ...
    def poll_move_events(self) -> None:
        ...
        while True:
            try:
                ...
                break
            changed = True
            self.add_recent(f"队列 {self.display_queue_event(event_name)}：{task.prefix}（{self.display_task_status(task.status)}）")
            if event_name == "moved":
                self.last_committed_transaction = task.transaction
                tx_id = getattr(task.transaction, "transaction_id", "")
                if tx_id in self.optimistic_transaction_ids:
                    self.optimistic_transaction_ids.remove(tx_id)
                elif self.fast_index is not None:
                ...
                tx_id = getattr(task.transaction, "transaction_id", "")
                self.optimistic_transaction_ids.discard(tx_id)
                self.add_issue("错误", task.prefix, "MOVE_FAILED", task.error, "检查恢复面板和文件系统；必要时重新打开该目录。")
    ...
    def update_queue_model(self) -> None:
        if self.move_runner is None:
            ...
            return
        rows = [
            [task.task_id, task.prefix, task.selected_stem, self.display_task_status(task.status), task.error]
    ...
    def display_queue_event(self, event_name: str) -> str:
        mapping = {
            "queued": "已入队",
            "started": "开始",
            "moved": "完成",
        ...
        return mapping.get(str(event_name), str(event_name))

    def update_issues_from_audit(self) -> None:
        rows: list[list[Any]] = []
        for issue in getattr(self.audit_summary, "blocking_errors", []) or []:
            rows.append(["错误", getattr(issue, "prefix", ""), getattr(issue, "error_code", ""), getattr(issue, "message", ""), getattr(issue, "suggested_action", "")])
        for issue in getattr(self.audit_summary, "warnings", []) or []:
            rows.append(["警告", "", "WARNING", issue, "方便时复核。"])
        if self.recovery_rows:
            rows.append(["错误", "", "RECOVERY_PENDING", f"{len(self.recovery_rows)} 个未完成事务", "先处理恢复事项，再启用文件移动。"])
    ...
    def update_formula(self) -> None:
        summary = (getattr(self.fast_index, "audit_counts", None) if self.fast_index else None) or self.audit_summary
        if summary is None:
            self.formula_text.setText("尚未载入校核结果。")
            return
        ...
        self.formula_text.setText(
            f"筛选目录：{getattr(summary, 'review_name', '')}\n"
            f"未完成：{root_images} 张图片 = {root_groups} 组 x {n}\n"
            f"完成区：out 图片 {out} = done 组 {done} x {max(n - 1, 0)}\n"
            f"标签同步：{self.display_label_sync(label_sync)}\n"
            f"审计状态：{STATUS}"
        )
    ...
    def update_kpis(self) -> None:
        ...
        self.kpi_labels["Label Sync"].setText(self.display_label_sync(str(getattr(summary, "label_sync_status", "PENDING"))))
        self.kpi_labels["Audit state"].setText("已完成" if self.audit_summary is not None else "构建中")

    def display_label_sync(self, status: str) -> str:
        mapping = {
            "PASS": "通过",
            "IN_PROGRESS_SYNCED": "进行中/已同步",
            "ERROR": "错误",
            "PENDING": "待校核",
            "FAIL": "错误",
        }
    ...
    def export_current_audit(self) -> None:
        if self.current_review_dir is None or self.core is None:
            self.show_disabled_reason("尚未打开筛选目录")
            return
        worker = GenericWorker(self.generation, lambda: self.core.export_target_audit_report(self.current_review_dir))
        worker.signals.progress.connect(self.on_open_progress)
        worker.signals.error.connect(self.on_worker_error)
        worker.signals.quick_ready.connect(lambda _gen, result: self.add_recent(f"校核报告已导出：{result}"))
        self.thread_pool.start(worker)
...
class GenericWorker(QRunnable):
    ...
    def run(self) -> None:
        try:
            self.signals.progress.emit(self.generation, "正在导出报告", "运行报告服务")
            result = self.fn()
...
def create_app(argv: list[str] | None = None) -> QApplication:
    ...
    app.setApplicationName("CIVL7009 图源人工筛选器")
    return app


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="CIVL7009 图源人工筛选器 V1.9.1 PySide6 Review Cockpit。")
    parser.add_argument("--core-path", default="", help="显式指定 V1.8.1 core 路径。")
    parser.add_argument("--developer-mode", action="store_true", help="允许开发模式 sibling core override。")
    parser.add_argument("--audit-only", action="store_true", help="只导出校核报告并退出。")
    parser.add_argument("--id-root", default="", help="--audit-only 使用的 ID 根目录。")
    parser.add_argument("--image-dir", default="", help="--audit-only 使用的单个图片筛选目录。")
