...
class ManualReviewError(RuntimeError):
    """Raised when a transformed-dataset review directory is unsafe for GUI processing."""
...
def export_audit_report(id_root: Path, output_root: Path | None = None) -> Path:
    ...
    run_dir = output_root / f"YOLO_Transformed_Dataset_Cleaning_Browser_Audit_V1.3_{timecode}"
...
def build_markdown_summary(id_root: Path, summaries: list[dict[str, Any]], timecode: str) -> str:
    ...
    return (
        "# YOLO Transformed Dataset Cleaning Browser Audit Summary\n\n"
        "## English\n"
        f"- Status: {STATUS}.\n"
...
class ManualReviewPickerApp:
    def __init__(self, id_root: Path | None = None, review_dir: Path | None = None) -> None:
        ...
        self.root = tk.Tk()
        self.root.title("YOLO Transformed Dataset Cleaning Browser V1.3")
    ...
    def label_index_for_review(self, review_dir: Path) -> dict[str, list[Path]]:
        ...
        if key not in self.label_index_by_review:
            ...
            validate_child_dirs(review_dir, label_review_dir, create_missing=True)
            self.label_index_by_review[key] = build_label_index(label_review_dir)
    ...
    def prepare_fast_transaction(self, prefix: str, image_path: Path) -> MoveTransaction:
        ...
        return prepare_transaction_from_members(
            self.current_review_dir,
            prefix,
            image_path,
            members,
            self.label_index_for_review(self.current_review_dir),
        )
    ...
    def update_queue_status(self) -> None:
        ...
        if failed:
            self.queue_status_var.set(f"后台队列已暂停: FAILED {failed}, MOVED {moved}, RUNNING {running}, QUEUED {queued}{dirty}")
        else:
            self.queue_status_var.set(f"后台队列: QUEUED {queued}, RUNNING {running}, MOVED {moved}{dirty}")
    ...
    def draw_card(self, image_path: Path, x: int, y: int, w: int, h: int) -> None:
        ...
        try:
            photo = self.preview_cache.get_photo(image_path, w - 16, preview_h - 10)
    ...
    def preload_next_group(self) -> None:
        ...
        if target_index < len(self.current_groups):
            ...
            card_h = max(240, (height - 60) // ((len(members) + cols - 1) // cols)) - 16
            preview_h = card_h - 58
            for image_path in members:
                try:
                    self.preview_cache.get_photo(image_path, card_w - 16, preview_h - 10)
                except Exception:
                    continue
    ...
    def schedule_choose_source(self, image_path: Path) -> None:
        ...
        self.pending_click_after = self.root.after(70, lambda p=image_path: self.run_scheduled_choice(p))
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="YOLO transformed-dataset cleaning browser with label-synchronised source-group cleanup.")
