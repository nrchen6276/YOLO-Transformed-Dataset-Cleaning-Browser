...
class ManualObjectsService:
    ...
    def list_groups(self) -> list[ManualObjectGroup]:
        ...
        if not self.root.exists() or not self.root.is_dir():
            return groups
        for reason_dir in sorted([p for p in self.root.iterdir() if p.is_dir() and not p.name.startswith("_")], key=lambda p: reason_sort_key(p.name)):
    ...
    def _load_from_csv(self, started: float) -> ManualObjectsIndexResult:
        ...
        with self.index_path.open("r", encoding="utf-8-sig", newline="") as handle:
            ...
            for row in reader:
                ...
                group_folder = str(row.get("group_folder", "") or "")
                bucket = str(row.get("duplicate_count_bucket", "") or "")
                ...
                if row.get("dataset_id"):
                    data["dataset_ids"].add(str(row["dataset_id"]))
                for label_class in normalise_label_class_set(row.get("label_class_set")):
                    data["label_classes"].add(label_class)
                if row.get("copy_status"):
                    data["copy_statuses"].add(str(row["copy_status"]))
        summaries = []
