...
class ThumbnailWorker(QRunnable):
    def __init__(
        ...
        max_size: tuple[int, int] = (430, 250),
    ) -> None:
        super().__init__()
        self.generation = generation
        ...
        try:
            ...
            with Image.open(self.image_path) as image:
                ...
                if self.draw_bboxes and self.label_path:
                    self.draw_yolo_bboxes(image, self.label_path, ImageDraw, ImageFont)
    ...
    @staticmethod
    def draw_yolo_bboxes(image: Any, label_path: Path, image_draw: Any, image_font: Any) -> None:
        ...
        for idx, (cls, cx, cy, bw, bh) in enumerate(boxes):
            ...
            line_width = max(2, int(min(width, height) / 180))
            for offset in range(line_width):
                draw.rectangle([x1 - offset, y1 - offset, x2 + offset, y2 + offset], outline=colour)
            label = f"class {cls}"
            text_bbox = draw.textbbox((x1, y1), label, font=font) if hasattr(draw, "textbbox") else (x1, y1, x1 + 54, y1 + 12)
            tx1, ty1, tx2, ty2 = text_bbox
            pad = 3
            bg_y1 = max(0, y1 - (ty2 - ty1) - pad * 2)
            draw.rectangle([x1, bg_y1, x1 + (tx2 - tx1) + pad * 2, bg_y1 + (ty2 - ty1) + pad * 2], fill=colour)
            draw.text((x1 + pad, bg_y1 + pad), label, fill="white", font=font)
...
class MainWindow(QMainWindow):
    def __init__(self, run_mode: str = "gui_production") -> None:
        ...
        self.manual_thumbnail_cache: dict[tuple[str, int, int, str, int, int, bool], bytes] = {}
    ...
    def manual_thumbnail_key(self, item: ManualObjectItem) -> tuple[str, int, int, str, int, int, bool]:
        try:
        ...
        try:
            ...
            label_size = 0
            label_mtime = 0
        return (str(item.image_path), image_size, image_mtime, str(item.label_path), label_size, label_mtime, self.manual_draw_bboxes)
    ...
    def request_manual_thumbnail(self, item: ManualObjectItem) -> None:
        ...
        worker = ThumbnailWorker(generation, item.item_id, item.image_path, item.label_path, self.manual_draw_bboxes)
...
def main(argv: list[str] | None = None) -> int:
    ...
    app.setApplicationName("CIVL7009 Source Group Picker V3.0.3")
