...
def build_fast_review_index(image_review_dir: Path, preview_limit: int | None = None) -> FastReviewIndex:
    ...
    for prefix, members in root_by.items():
        if prefix.startswith("__NO_RF__::"):
            issues.append(AuditIssue("ERROR", "ERROR_MISSING_RF_PREFIX", prefix, [path.name for path in members], "Missing .rf. prefix.", "Rename or remove the affected image.", True))
    ...
    summary.can_select = bool(valid_prefixes) and group_size > 1 and not issues
    ...
    index = FastReviewIndex(
        ...
        ready_for_commit=bool(valid_prefixes) and not issues and preview_limit is None,
        blocking_errors=issues,
    )
    log_perf(
    ...
    try:
        with os.scandir(image_review_dir) as entries:
            for entry in entries:
                ...
                if len(groups[prefix]) == group_size and prefix not in complete_prefix_set:
