...
from civl7009_picker_v2_1.app import MainWindow
from civl7009_picker_v2_1.assets import AssetService
from civl7009_picker_v2_1.core import PickerCoreFacade
from civl7009_picker_v2_1.paths import ASSET_DIR, PROGRAMME_DIR
...
class V21PickerTests(unittest.TestCase):
    @classmethod
    ...
    def test_empty_state_has_directory_entry_points(self) -> None:
        ...
        try:
            self.assertEqual(window.choose_id_btn.text(), "选择 ID 根目录")
            self.assertEqual(window.choose_review_btn.text(), "直接选择筛选目录")
            self.assertIn("请选择", window.progress_chip.text())
        finally:
            window.close()
